...
(async()=>{
  ...
  try{
    ...
    await test('server/discover와 tools/list가 modern MCP 2026-07-28로 응답한다',async()=>{
      ...
      assert.equal(discover.data.result._meta['io.modelcontextprotocol/serverInfo'].version,'1.8.2.1');
    ...
    await test('reality.discover/observe가 Graph와 State/Event를 읽기 전용으로 노출한다',async()=>{
      ...
      assert.equal(ds.ok,true); assert.equal(ds.reality_layer_version,'1.8.2.1'); assert.ok(ds.graph.nodes.length>0); assert.ok(ds.devices.length>0);
